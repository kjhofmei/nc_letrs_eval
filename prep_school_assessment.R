# Prepare school assessment data
...
# prep data ----
  ...
  ## prep school assessment data (non-charter district level)
    ...
    district_assessment = rbind(sa_18[lea_code != school_code & lea_name != 'Charter Schools'],
                                sa_19[lea_code != school_code & lea_name != 'Charter Schools'],
                                sa_21[lea_code != school_code & lea_name != 'Charter Schools'],
                                sa_22[lea_code != school_code & lea_name != 'Charter Schools'],
                                sa_23[lea_code != school_code & lea_name != 'Charter Schools'],
                                fill = TRUE)
    ...
    ### impute censored glp_pct values
      ...
      pct_cols = names(district_assessment)[names(district_assessment) %like% 'pct']
      for(c in pct_cols){
        district_assessment[, paste0(c, '_num') := as.numeric(get(c))]
      }
      
      ### inferred from available data
      district_assessment[!is.na(as.numeric(glp_pct)), glp_pct_inf := as.numeric(glp_pct)]
      
      ### if lev3 - lev5 pct not missing, replace
      district_assessment[is.na(glp_pct_inf) & !is.na(lev3_pct_num) & !is.na(lev4_pct_num) & !is.na(lev5_pct_num), glp_pct_inf := lev3_pct_num + lev4_pct_num + lev5_pct_num]
      
      ### if lev4, lev5 sum to 1 (or more), replace with 1
      district_assessment[is.na(glp_pct_inf) & lev4_pct_num + lev5_pct_num >= 100, glp_pct_inf := 100]
      
      ### if lev1, lev2 nonmissing, replace with 1 - ...
      district_assessment[is.na(glp_pct_inf) & !is.na(lev1_pct_num) & !is.na(lev2_pct_num), glp_pct_inf := 100 - lev1_pct_num - lev2_pct_num]
      
      ### create min imputation
      district_assessment[, glp_pct_min_imp := glp_pct_inf]
      district_assessment[is.na(glp_pct_min_imp) & glp_pct == '>95' & lev4_pct_num + lev5_pct_num >= 95, glp_pct_min_imp := lev4_pct_num + lev5_pct_num]
      district_assessment[is.na(glp_pct_min_imp) & glp_pct == '>95', glp_pct_min_imp := 95]
      district_assessment[is.na(glp_pct_min_imp) & glp_pct == '<5', glp_pct_min_imp := 0]
      
      ### create max imputation
      district_assessment[, glp_pct_max_imp := glp_pct_inf]
      district_assessment[is.na(glp_pct_max_imp) & glp_pct == '>95', glp_pct_max_imp := 100]
      district_assessment[is.na(glp_pct_max_imp) & glp_pct == '<5', glp_pct_max_imp := 5]
  ...
  assessment = rbind(district_assessment,
                      charter_assessment[, .(lea_code = 'CH', school_code, reporting_year, subject, den, glp_pct_min_imp, glp_pct_max_imp)],
                      fill = TRUE)
  
  ### fill in missing values of glp_pct for districts
  assessment[is.na(glp_pct_min_imp), glp_pct_min_imp := as.numeric(glp_pct)]
  assessment[is.na(glp_pct_max_imp), glp_pct_max_imp := as.numeric(glp_pct)]
  
  ### update names to match other data
  setnames(assessment, c('reporting_year'), c('year'))
  
  ### keep only cols we need
  keep_cols = c('year', 'school_code', 'lea_code', 'subject', 'den', 'glp_pct_min_imp', 'glp_pct_max_imp')
  drop_cols = c('lea_name', 'school_code', 'school_name', 'grade_span', 'title_1', 'subgroup', 'total_pct', '...22')
  assessment = assessment[, ..keep_cols]
    
# save data ----
fwrite(assessment, paste0(workdatadir, 'prep_school_assessment.csv'))